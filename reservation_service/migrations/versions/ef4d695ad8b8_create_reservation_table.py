...
def upgrade():
    ...
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], name='reservation_user_fkey', onupdate='NO ACTION', ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name='reservation_pkey')
    )
    with op.batch_alter_table('reservation', schema=None) as batch_op:
        batch_op.create_index('ix_reservation_user_id_fkey', ['user_id'], unique=False)
